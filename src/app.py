    ...
    try:
        if await aredis.get("state") == b"running":
            if await aredis.get("processed") != await aredis.get("lastProcessed"):
                await aredis.set(
                    "percent",
                    round(int(await aredis.get("processed")) / int(await aredis.get("length_of_work")) * 100, 2,),
                )
...
async def index():
    return (
        'This is the index page. Try the following to <a href="'
        + url_for("start_work")
        + '">start some test work</a> with a progress indicator.'
    )
